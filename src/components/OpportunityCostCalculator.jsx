              ...
              <svg className="cash-flow-plot" viewBox={`0 0 ${Math.max(years * 50, 1000)} 520`} preserveAspectRatio="none">
                ...
                {maxNegativeA > 0 ? (
                  <>
                    ...
                    {/* Y-axis labels - proportional scaling (Option A) */}
                    {(() => {
                      // Calculate proportional positions based on max absolute value
                      const negativeRatio = maxNegativeA / maxAbsoluteA
                      const positiveRatio = maxPositiveA / maxAbsoluteA
                      
                      return [
                        {ratio: -1, value: -maxNegativeA, actualRatio: -negativeRatio},
                        {ratio: -0.5, value: -maxNegativeA / 2, actualRatio: -negativeRatio / 2},
                        {ratio: 0, value: 0, actualRatio: 0},
                        {ratio: 0.5, value: maxPositiveA / 2, actualRatio: positiveRatio / 2},
                        {ratio: 1, value: maxPositiveA, actualRatio: positiveRatio}
                      ].map((item, idx) => {
                        // Calculate Y position: proportional to max absolute value
                        let y
                        if (item.actualRatio < 0) {
                          // Negative side: proportional range from 250
                          y = 250 + (Math.abs(item.actualRatio) * 210)
                        } else if (item.actualRatio > 0) {
                          // Positive side: proportional range from 250
                          y = 250 - (item.actualRatio * 210)
                        } else {
                          y = 250 // Zero line
                        }
                        return (
                ...
                {(() => {
                  ...
                  const maxBarHeight = maxNegativeA > 0 ? 210 : 420 // Full height if no negatives
                  ...
                  cashFlowsA.forEach(cf => {
                    ...
                    // Use proportional scaling based on max absolute value
                    if (cf.isPositive) {
                      barHeight = maxAbsoluteA > 0 ? (cf.value / maxAbsoluteA) * maxBarHeight : 0
                      y = zeroY - barHeight
                      height = barHeight
                    } else {
                      barHeight = maxAbsoluteA > 0 ? (Math.abs(cf.value) / maxAbsoluteA) * maxBarHeight : 0
                      y = zeroY
                      height = barHeight
                    }
                  ...
                  return cashFlowsA.map((cf, idx) => {
                    ...
                    // Calculate bar dimensions with proportional scaling (Option A)
                    let barHeight, y, height
                    // Use proportional scaling based on max absolute value
                    if (cf.isPositive) {
                      barHeight = maxAbsoluteA > 0 ? (cf.value / maxAbsoluteA) * maxBarHeight : 0
              ...
              <svg className="cash-flow-plot" viewBox={`0 0 ${Math.max(years * 50, 1000)} 520`} preserveAspectRatio="none">
                ...
                {maxNegativeB > 0 ? (
                  <>
                    ...
                    {/* Y-axis labels - proportional scaling (Option B) */}
                    {(() => {
                      // Calculate proportional positions based on max absolute value
                      const negativeRatio = maxNegativeB / maxAbsoluteB
                      const positiveRatio = maxPositiveB / maxAbsoluteB
                      
                      return [
                        {ratio: -1, value: -maxNegativeB, actualRatio: -negativeRatio},
                        {ratio: -0.5, value: -maxNegativeB / 2, actualRatio: -negativeRatio / 2},
                        {ratio: 0, value: 0, actualRatio: 0},
                        {ratio: 0.5, value: maxPositiveB / 2, actualRatio: positiveRatio / 2},
                        {ratio: 1, value: maxPositiveB, actualRatio: positiveRatio}
                      ].map((item, idx) => {
                        // Calculate Y position: proportional to max absolute value
                        let y
                        if (item.actualRatio < 0) {
                ...
                {(() => {
                  ...
                  const maxBarHeight = maxNegativeB > 0 ? 210 : 420 // Full height if no negatives
                  ...
                  cashFlowsB.forEach(cf => {
                    ...
                    // Use proportional scaling based on max absolute value
                    if (cf.isPositive) {
                      barHeight = maxAbsoluteB > 0 ? (cf.value / maxAbsoluteB) * maxBarHeight : 0
                      y = zeroY - barHeight
                      height = barHeight
                    } else {
                      barHeight = maxAbsoluteB > 0 ? (Math.abs(cf.value) / maxAbsoluteB) * maxBarHeight : 0
                      y = zeroY
                      height = barHeight
                    }
                    const valueY = cf.isPositive ? y - 10 : y + height + 20
                  ...
                  return cashFlowsB.map((cf, idx) => {
                    ...
                    // Calculate bar dimensions with proportional scaling (Option B)
                    let barHeight, y, height
                    // Use proportional scaling based on max absolute value
                    if (cf.isPositive) {
                      barHeight = maxAbsoluteB > 0 ? (cf.value / maxAbsoluteB) * maxBarHeight : 0
            ...
            <div className="breakdown-table">
              ...
              {breakdownA.map((row, idx) => (
                <div key={idx} className="breakdown-row">
                  ...
                  <span style={{color: '#333'}}>${row.netWorth.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                  <span style={{color: '#333'}}>${row.valueWithoutInterest.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
              ...
              <div className="breakdown-row final">
                <span>Final</span>
                <span>${futureValueA.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                <span>-</span>
                <span>-</span>
                <span>-</span>
                <span>-</span>
          ...
          <div className="breakdown-card">
            ...
            <div className="breakdown-table">
              ...
              {breakdownB.map((row, idx) => (
                <div key={idx} className="breakdown-row">
                  <span>{row.year}</span>
                  <span>${row.netWorth.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                  <span>${row.valueWithoutInterest.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                  <span className={row.interestGainLoss >= 0 ? 'positive' : 'negative'}>
                    ${row.interestGainLoss.toLocaleString(undefined, {maximumFractionDigits: 0})}
                  </span>
                  <span className={row.cashFlowGainLoss >= 0 ? 'positive' : 'negative'}>
              ...
              <div className="breakdown-row final">
                <span>Final</span>
                <span>${futureValueB.toLocaleString(undefined, {maximumFractionDigits: 0})}</span>
                <span>-</span>
                <span>-</span>
                <span>-</span>
                <span>-</span>
